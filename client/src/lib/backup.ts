...
import { BACKUP_FORMAT, BACKUP_FORMAT_VERSION } from '@hpt/shared';

/*
 * Re-exported so this module stays the one place the panel and its tests import
 * backup vocabulary from, while the values themselves have a single definition
 * in `@hpt/shared` that the server imports too. They were briefly duplicated
 * across the fence and guarded by a parity test, back when `server/src` could
 * not take a runtime value from that package; the add-on image now stages it,
