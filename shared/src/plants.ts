...
export interface PlantCatalogueEntry {
  /** Canonical display name, shown in pickers. */
  name: string;
  /** One of `SEED_CATEGORIES`. */
  category: string;
  /**
   * Other names people type for the same plant: regional names, common
   * cultivars, and the compound names that would otherwise resolve to the
   * wrong plant. "Cape gooseberry" lives here on the ground cherry rather than
   * anywhere near the gooseberries, because that is what it actually is.
   */
 ...
 */
export const PLANT_CATALOGUE: readonly PlantCatalogueEntry[] = [
  // ---------------------------------------------------------------- Nightshade
...
function pluralForms(word: string): string[] {
  ...
  if (word.endsWith('o')) return [`${word}es`, `${word}s`];
...
export function plantLookupKeys(entry: PlantCatalogueEntry): string[] {
  const keys = new Set<string>();

  for (const key of [entry.name, ...(entry.aliases ?? [])]) {
    for (const variant of keyVariants(key)) keys.add(variant);
  }
 ...
 */
const INDEX: Map<string, PlantCatalogueEntry> = (() => {
  const index = new Map<string, PlantCatalogueEntry>();
...
/** How sure the catalogue is that it recognised what she typed. */
export type PlantMatchConfidence =
  /** The whole variety name is a catalogue entry: "Jalapeño", "Cherry Tomato". */
  | 'exact'
  /** A known plant was found inside a longer name: "Sugar Baby Watermelon". */
  | 'contains';

export interface PlantMatch {
  entry: PlantCatalogueEntry;
  confidence: PlantMatchConfidence;
  /** The normalised span of the input that matched, for explaining the result. */
  matched: string;
}
 ...
 */
export function matchPlant(variety: string | null | undefined): PlantMatch | null {
  if (typeof variety !== 'string') return null;
  ...
  if (exact) return { entry: exact, confidence: 'exact', matched: whole };

  let best: { entry: PlantCatalogueEntry; length: number; end: number; matched: string } | null =
    null;
  ...
  for (let start = 0; start < tokens.length; start += 1) {
    for (let end = start + 1; end <= tokens.length; end += 1) {
      ...
      const isBetter =
        best === null || length > best.length || (length === best.length && end > best.end);

      if (isBetter) best = { entry, length, end, matched: span };
    }
  }
...
export function categoryForVariety(variety: string | null | undefined): string | null {
 ...
 */
export function searchPlants(query: string, limit = 12): PlantCatalogueEntry[] {
  const needle = normalizePlantName(query);
  if (needle === '') return [];

  const ranked: { entry: PlantCatalogueEntry; rank: number }[] = [];
  ...
  for (const entry of PLANT_CATALOGUE) {
    ...
    else if (aliases.some((alias) => alias.includes(needle))) rank = 3;
