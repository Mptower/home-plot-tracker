...
import { BACKUP_FORMAT, BACKUP_FORMAT_VERSION } from './backup/format.ts';
...
export function validateSettings(
  raw: unknown,
  path = 'body',
): ValidationResult<GardenSettings> {
  const collector = new Collector();
...
/** A backup file that passed every check, ready to be applied. */
export interface ValidatedBackup {
  snapshot: GardenSnapshot;
  /** Absent in a bare `ha-tools` snapshot, in which case settings are left alone. */
  settings: GardenSettings | null;
  /** What the file claims about itself. `null` when it does not say. */
  exportedAt: string | null;
  formatVersion: number;
  /** Whether the file carried our `format` marker. */
  labelled: boolean;
}
 ...
 *
 * `unsupported` is separated from `invalid` because they need different words.
 * "Your file has a problem on line 40" is actionable; saying that about a file
 * written by a *newer* version of the app is a lie that sends her editing JSON
 * to fix something that was never broken.
 */
export type BackupValidation =
  | { ok: true; value: ValidatedBackup }
  | { ok: false; kind: 'invalid'; issues: ValidationIssue[] }
...
function notABackup(message: string): BackupValidation {
 ...
 */
export function validateBackupDocument(raw: unknown): BackupValidation {
  if (!isPlainObject(raw)) {
    return notABackup(
      `expected a backup file containing a garden, received ${describe(raw)}`,
    );
  }
  ...
  if (raw.format !== undefined) {
    if (typeof raw.format !== 'string' || raw.format !== BACKUP_FORMAT) {
      return {
        ...
        kind: 'unsupported',
        message:
          `This file identifies itself as ${JSON.stringify(raw.format)}, ` +
  ...
  if (raw.formatVersion !== undefined) {
    if (
      ...
      return notABackup(
        `formatVersion should be a whole number, received ${describe(raw.formatVersion)}`,
      );
    }
    ...
    if (raw.formatVersion > BACKUP_FORMAT_VERSION) {
      return {
        ...
        kind: 'unsupported',
        message:
          `This backup is in format version ${raw.formatVersion}, and this version of ` +
  ...
  if (!hasAnyCollection) {
    return notABackup(
      'this file contains no seeds, beds or harvests, so it is not a garden backup. ' +
        'Nothing has been changed.',
    );
  }
  ...
  for (const field of SNAPSHOT_FIELDS) {
    if (raw[field] === undefined) {
      collector.add(
        `file.${field}`,
        'required field is missing — a restore replaces the whole garden, so an ' +
