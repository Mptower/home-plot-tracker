...
export function severityFor(lowF) {
    if (lowF <= FROST_THRESHOLDS_F.hardFreeze)
        return 'hard_freeze';
    if (lowF <= FROST_THRESHOLDS_F.frost)
        return 'frost';
    if (lowF <= FROST_THRESHOLDS_F.advisory)
        return 'advisory';
    return 'none';
}
 ...
 */
export function nightOf(at, precision) {
    ...
    for (const row of Array.isArray(bed.layout) ? bed.layout : []) {
        for (const cell of Array.isArray(row) ? row : []) {
            ...
            if (tenderness === 'unknown')
                unknownSquares += 1;
            if (!seen.has(variety))
                seen.set(variety, tenderness);
        }
    }
    const byTenderness = { tender: [], hardy: [], unknown: [] };
 ...
 * to stay quiet.
 */
export function assessFrostRisk(input) {
    ...
    for (const point of input.forecast) {
        if (!Number.isFinite(point.lowF))
            continue;
        if (point.lowF > FROST_THRESHOLDS_F.advisory)
            continue;
        const at = new Date(point.at).getTime();
        if (Number.isNaN(at) || at < now.getTime() || at > horizon)
            continue;
    ...
    const [night, coldest] = [...nights.entries()].reduce((best, candidate) => candidate[1].lowF < best[1].lowF ||
    ...
    const bedsAtRisk = tallies.filter((tally) => band === 'hard_freeze'
        ? tally.byTenderness.tender.length + tally.byTenderness.hardy.length > 0
        : tally.byTenderness.tender.length > 0);
    const severity = bedsAtRisk.length > 0 ? band : 'none';
    const distinct = (lists) => [...new Set(lists.flat())].sort((left, right) => left.localeCompare(right));
    const named = bedsAtRisk.map((tally) => ({
        bedId: tally.bedId,
