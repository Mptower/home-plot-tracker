...
export class HomeAssistantClient {
    ...
    #request(path, init) {
        return this.#send(`${this.#baseUrl}${path}`, init);
    }
    /**
     ...
     */
    async getSelfSlug() {
        const result = await this.#send(`${this.#supervisorUrl}/addons/self/info`, { method: 'GET' });
        if (!result.ok)
            return null;
    ...
    callService(domain, service, data, returnResponse = false) {
        ...
        return this.#request(`/services/${encodeURIComponent(domain)}/${encodeURIComponent(service)}${query}`, { method: 'POST', body: data });
    }
}
