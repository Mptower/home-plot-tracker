...
`shared/` carries runtime values for **both** sides now. The plant catalogue in
[`shared/src/plants.ts`](shared/src/plants.ts) is the largest — the client
bundles it through Vite, and `server/src/ha/varietyCategory.ts` imports it to
resolve what is planted in a bed square. The category → tenderness map and the
two backup format constants (`BACKUP_FORMAT`, `BACKUP_FORMAT_VERSION`) are the
others.
[`server/test/shared-imports.test.ts`](server/test/shared-imports.test.ts) is
what makes that safe: rather than forbidding the imports, it guards the staging
that keeps them resolvable, because every way the staging can silently stop
happening is a way her add-on stops booting.
...
- `PLANT_CATALOGUE` and the matcher (`matchPlant`, `categoryForVariety`,
  ...
  derived from the variety she typed rather than asked for. Used on **both**
  sides: the seed vault reads it directly, and `server/src/ha/varietyCategory.ts`
  imports it at runtime to resolve what is planted in a bed square. That runtime
  import is only safe because the add-on image stages `@hpt/shared` and links it
  as a `file:` dependency — `server/test/shared-imports.test.ts` is what keeps
  that staging honest, and every way it can silently stop happening is a way the
  add-on stops booting.
